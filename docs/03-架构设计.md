...
flowchart TD
    ...
    RET --> VEC["Chroma 576: 434 QA + 142 chunk"]
    RET --> BM25["BM25 576: 434 QA + 142 chunk"]
...
- Chroma 与 BM25 都覆盖 QA/chunk 双数据源，两个索引的现行总量均为 576。本地 Chroma 固定 `0.4.24`，collection 使用 `hnsw:num_threads=1` 和 `hnsw:batch_size=3` 保持 Windows 本地构建稳定。
...
5. 若 top1 metadata 中存在 `fixed_answer`，直接返回固定话术，跳过 LLM 改写与 grounding（ADR 0024）
6. Prompt 组装 (app/rag/prompts.py)：
   a. 加载系统提示词（SYSTEM_PROMPT）
   b. 把检索结果格式化为 kb_results
   c. 拼接用户问题（USER_TEMPLATE）
7. 调用 LLM 三级路由（app/rag/llm.py）
8. 检测 LLM 是否触发"未找到"模板 → 切换 mode
9. RAG 命中时追加 grounding 锚点；成功轮次写入 conversations.db
10. 后端把结果（含 sources、mode、retrieval_score、conversation_id）返回 H5
11. H5 渲染：答案 + 参考资料（可展开）+ 反馈按钮
```
...
### 3.8 GET /api/admin/usage/trend

返回管理端使用频率趋势。`days` 仅支持 `7` 或 `30`；统计单位为每次 `/api/chat` 请求，按 UTC+8 自然日聚合且包含当天。
...
系统管理员 `scope=null` 且返回全量；业务管理员由 `region_scope` 自动限定辖区。完整序列包含缺失日期补零。
...
| `api/usage_trend_admin.py` | 7/30 日使用频率趋势，业务管理员自动按 `region_scope` 限定 |
...
| `persistence/usage_trend.py` | `query_log` 按 UTC+8 自然日聚合，生成 7/30 日补零序列 |
...
| 文件 | 职责 |
|------|------|
| `index.html` | 聊天主页面 SPA；历史会话支持显式「开始新对话」 |
| `login.html` | 手机号白名单登录页 |
| `dashboard.html` | 统一数据看板（KB 复核队列 / 使用监测 + 使用频率折线图）；白名单和测验经顶部统一导航进入 |
| `whitelist.html` | 白名单管理页（CRUD + 手机号姓名搜索 + 标准调查点级联选择 + 翻页；CSV 导入已停用） |
...
│   ├── backend/data/bm25_index.json  BM25（双源 576 = 434 QA + 142 chunk）
