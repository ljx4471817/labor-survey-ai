...
│         https://xxx.trycloudflare.com  →  :8000              │
└──────────────────────────┬──────────────────────────────────┘
                           │ HTTPS / WSS（出站连接，无需端口映射）
                           ▼
┌─────────────────────────────────────────────────────────────┐
│           本地电脑（用户机器，常驻运行）                          │
│  ┌──────────────────────────────────────────────────────┐  │
...
│  │  │   RAG 引擎（app/rag/）                     │       │  │
│  │  │  ┌──────────┐ ┌──────────┐ ┌──────────┐  │       │  │
│  │  │  │retriever │ │bm25      │ │prompts   │  │       │  │
│  │  │  │(hybrid)  │ │(rank_bm25)│ │/llm     │  │       │  │
│  │  │  └──────────┘ └──────────┘ └──────────┘  │       │  │
│  │  └──────────────────────────────────────────────┘   │  │
│  │  ┌──────────────────────────────────────────────┐   │  │
│  │  │   H5 前端（backend/static/index.html）         │   │  │
│  │  │   单文件 SPA：聊天 / 反馈                       │   │  │
│  │  │   （语音按钮 2026-06-21 起已隐藏）              │   │  │
│  │  └──────────────────────────────────────────────┘   │  │
│  └──────────────────────────────────────────────────────┘  │
│                           │                                  │
│  ┌────────────────────────┼────────────────────────┐       │
...
```
1. 用户对答案点击"采纳 / 不采纳"按钮
2. H5 fetch POST /api/feedback
   body: { question, answer, mode, rating: "up"|"down", comment, sources }
3. 后端 (app/api/feedback.py) 追加写入 data/feedback.jsonl
4. 返回 { ok: true, record_id: "..." }
5. H5 显示"已记录 (record_id)"
```
...
| `api/chat.py` | `/api/chat` 路由，越界/模糊/检索/LLM 编排 |
| `api/voice.py` | **（2026-06-21 已停用）** `/api/voice/stream` WebSocket，浏览器音频与讯飞的桥接 |
| `api/_xunfei_auth.py` | **（2026-06-21 已停用）** 讯飞实时转写鉴权（生成签名 URL、token 缓存） |
| `api/feedback.py` | `/api/feedback` 接收采纳/不采纳，写入 feedback.jsonl |
| `rag/retriever.py` | 越界/模糊判断 + Chroma 检索 |
| `rag/bm25.py` | BM25 索引加载、关键词打分（hybrid 用） |
| `rag/llm.py` | DeepSeek / DashScope LLM 调用封装 |
...
| 文件 | 职责 |
|------|------|
| `index.html` | 单文件 SPA，含全部 CSS/JS |

UI 模块（全部内联在 index.html）：
- 聊天列表（用户气泡 / 助手气泡 / mode 标签 / 参考资料展开）
- 输入区（textarea + 发送按钮 + 麦克风按钮）
- 反馈区（采纳/不采纳按钮 + 评论输入框）
- 状态指示（在线/连接中/不可达）
...
│   ├── cmd 窗口 1: uvicorn app.main:app --reload --port 8000
│   └── cmd 窗口 2: cloudflared tunnel --url http://localhost:8000
│       └── 输出 https://xxx.trycloudflare.com 链接
│
├── 浏览器打开链接 → H5 聊天页
│   └── fetch /api/chat, /api/feedback
│   └── ~~WebSocket /api/voice/stream~~ **（2026-06-21 已停用）**
│
├── 数据文件
│   ├── backend/data/chroma/  向量库
│   ├── backend/data/bm25_index.json  BM25
│   └── backend/data/feedback.jsonl  反馈日志
│
└── .env  环境变量（API Keys 等）
```
...
- 关 cmd 窗口 1（uvicorn）→ 后端停
- 关 cmd 窗口 2（tunnel）→ 公网入口停
- 端口 8000 自动释放
