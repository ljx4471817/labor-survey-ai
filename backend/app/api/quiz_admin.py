...
def quiz_extract(req: ExtractRequest, phone: str = Depends(require_admin)) -> dict:
...
def quiz_extract_status(task_id: str, phone: str = Depends(require_admin)) -> dict:
...
def quiz_keypoints(quiz_id: str = Query(...), phone: str = Depends(require_admin)) -> dict:
    return {"items": quiz_db.list_keypoints(quiz_id)}


@router.post("/quiz/keypoint/review")
def quiz_keypoint_review(req: KeypointReviewRequest, phone: str = Depends(require_admin)) -> dict:
...
def quiz_generate_status(task_id: str, phone: str = Depends(require_admin)) -> dict:
    return _task_status(task_id)
...
def quiz_questions(quiz_id: str = Query(...), phone: str = Depends(require_admin)) -> dict:
    return {"items": quiz_db.list_questions(quiz_id)}


@router.post("/quiz/question/review")
def quiz_question_review(req: QuestionReviewRequest, phone: str = Depends(require_admin)) -> dict:
...
def quiz_targets(q: str | None = Query(None, max_length=30), phone: str = Depends(require_admin)) -> dict:
    """下发对象：全部 active 白名单用户，按 市 → 角色 分组（含各角色总人数；明细随响应返回，前端折叠展示）。"""
    users = whitelist_db.list_all(active_only=True)
    if q:
        qq = q.strip()
...
def _stats_rows(quiz_id: str, region: str | None) -> tuple[list[dict], dict, int, int]:
...
def quiz_whoami(phone: str = Depends(require_admin)) -> dict:
    user = whitelist_db.get_user(phone)
...
def quiz_stats(
    ...
    if not quiz:
        raise HTTPException(404, "测验不存在")
    rows, by_region, started, completed = _stats_rows(quiz_id, region)
    base_total = len(rows)
...
def quiz_stats_export(
    ...
    if not quiz:
        raise HTTPException(404, "测验不存在")
    rows, _, _, _ = _stats_rows(quiz_id, region)
    if q:
        qq = q.strip()
        rows = [r for r in rows if qq in r["name"] or qq in r["phone"]]
...
def quiz_kb_search(q: str = Query(..., min_length=1, max_length=50), phone: str = Depends(require_admin)) -> dict:
