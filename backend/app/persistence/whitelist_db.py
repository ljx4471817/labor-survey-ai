...
# PG 主键策略：GENERATED BY DEFAULT AS IDENTITY（非 ALWAYS）——
# 迁移脚本需显式写入从 SQLite 带来的 id，ALWAYS 会拒绝显式插入。
...
def _needs_migration(conn: "db.Connection") -> bool:
    cols = {r['name'] for r in db.table_columns(conn, 'whitelist')}
...
def _migrate(conn: "db.Connection") -> None:
    ...
    audit_schema = _AUDIT_SCHEMA_PG if conn.backend == 'postgres' else _AUDIT_SCHEMA_SQLITE
    conn.executescript(audit_schema)
    conn.commit()


def _db_target() -> str:
    """LSX_DB_WHITELIST 优先（PG DSN 或 SQLite 路径），未设置回落默认路径。"""
