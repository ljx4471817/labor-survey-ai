...
# PG 主键策略：GENERATED BY DEFAULT AS IDENTITY —— 迁移脚本需显式写入
# SQLite 带来的 id（GENERATED ALWAYS 会拒绝显式插入）。
...
# INSERT OR IGNORE 双套 SQL：PG 用 ON CONFLICT DO NOTHING。
# 不指定冲突目标列即可捕获任意唯一约束冲突，语义与 SQLite INSERT OR IGNORE 等价；
# 对应约束：targets 主键 (quiz_id, phone)、answers UNIQUE(quiz_id, phone, q_id)、
# scenes 主键 name。
_SQL_INSERT_TARGET = {
    "sqlite": (
    ...
    "postgres": (
        "INSERT INTO targets (quiz_id, phone, added_at) VALUES (?, ?, ?) "
...
_SQL_INSERT_ANSWER = {
    ...
    "postgres": (
        "INSERT INTO answers (quiz_id, phone, q_id, selected, correct, ts) "
        "VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING"
    ),
}
_SQL_INSERT_SCENE = {
    "sqlite": (
        "INSERT OR IGNORE INTO scenes (name, sort_order, active, created_at) "
        "VALUES (?, ?, 1, ?)"
    ),
    "postgres": (
        "INSERT INTO scenes (name, sort_order, active, created_at) "
        "VALUES (?, ?, 1, ?) ON CONFLICT DO NOTHING"
    ),
}


def _insert_sql(templates: dict, conn: "db.Connection") -> str:
    """按当前后端取对应方言的 INSERT SQL。"""
    return templates["postgres"] if conn.backend == "postgres" else templates["sqlite"]


def _migrate_month_nullable(conn: "db.Connection") -> None:
    ...
    for table, ddl, copy in (
        ...
        cols = {c["name"]: c["notnull"] for c in db.table_columns(conn, table)}
...
def _db_target() -> str:
    """LSX_DB_QUIZ 优先（PG DSN / SQLite 路径），回落 QUIZ_DB_PATH / 默认路径。"""
    return os.environ.get("LSX_DB_QUIZ") or str(DB_PATH)


def _get_conn() -> "db.Connection":
    """当前线程懒创建并复用连接包装（底层连接线程局部，由适配层管理）。"""
...
def set_targets(quiz_id: str, phones: list[str]) -> int:
    ...
    with _WRITE_LOCK:
        ...
        for phone in phones:
            conn.execute(
                _insert_sql(_SQL_INSERT_TARGET, conn),
...
def submit_answer(quiz_id: str, phone: str, q_id: str, selected: str, correct: bool) -> str:
    ...
    with _WRITE_LOCK:
        conn = _get_conn()
        cur = conn.execute(
            _insert_sql(_SQL_INSERT_ANSWER, conn),
...
def ensure_default_scenes() -> None:
    ...
    with _WRITE_LOCK:
        ...
        for i, name in enumerate(QUIZ_DEFAULT_SCENES, start=1):
            conn.execute(
                _insert_sql(_SQL_INSERT_SCENE, conn),
                (name, i, _now()),
            )
        conn.commit()
